            ...
            Action::make('makePayment')
                ...
                ->form([
                        ...
                        ->options(fn (callable $get) => Sale::where('client_id', $get('clientId'))
                            ...
                            ->mapWithKeys(fn ($sale) => [
                                $sale->id => new HtmlString(
                                    'Sale #' . $sale->id .
                                    ' - ' . number_format($sale->remaining_amount, 2) . ' ' . __('Currency') . ' remaining' .
                    ...
                    TextInput::make('paymentAmount')
                        ->label('Payment Amount (' . __('Currency') . ')')
                        ->numeric()
                        ->required()
                        ->minValue(0.01)
                            ...
                            function (callable $get) {
                                return function (string $attribute, $value, $fail) use ($get) {
                                    ...
                                    if ($sale && $value > $sale->remaining_amount) {
                                        $fail("Payment amount cannot exceed remaining amount of " . number_format($sale->remaining_amount, 2) . ' ' . __('Currency'));
                                    }
                                };
                            }
                        ]),
                    ...
                    Placeholder::make('paymentHistory')
                        ...
                        ->content(function (callable $get) {
                            ...
                            foreach ($payments as $payment) {
                                ...
                                $history .= '<span class="font-medium">' . __('Currency') . ' ' . number_format($payment['amount'], 2) . '</span>';
                                $history .= '</div>';
                            }
                ...
                ->action(function (array $data) {
                    ...
                    if ($data['paymentAmount'] > $sale->remaining_amount) {
                        Notification::make()
                            ...
                            ->body("Payment cannot exceed remaining amount of " . number_format($sale->remaining_amount, 2) . ' ' . __('Currency'))
                            ->danger()
                            ->send();
                        return;
                    }
                    ...
                    if ($success) {
                        ...
                        Notification::make()
                            ...
                            ->body(
                                "Payment of " . __('Currency') . ' ' . number_format($data['paymentAmount'], 2) . " recorded.\n" .
                                "Remaining: " . __('Currency') . ' ' . number_format($status['remaining_amount'], 2) . "\n" .
